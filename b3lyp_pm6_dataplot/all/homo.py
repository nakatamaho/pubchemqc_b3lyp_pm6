...
plt.xlabel("HOMO by PM6//PM6 in eV")
plt.ylabel("HOMO by B3LYP/6-31G*//PM6 in eV")
#plt.xlim([0,17])
#plt.ylim([0,15])

X = df['pm6_homo'].to_numpy().reshape(-1,1)
Y = df['b3lyp_pm6_homo'].to_numpy().reshape(-1,1)
...
plt.text(-15, -0, r'$y='+ str(round(a[0],3)) +' x + ' + str(round(b[0],3)) +'$' , horizontalalignment='left',fontsize=32)
plt.text(-15, -1.5, r'$r^{2} = ' + str(round(c,3)) +'$' , horizontalalignment='left',fontsize=32)
...
plt.title('PubChemQC B3LYP/6-31G*//PM6 HOMO')
plt.savefig(basename + ".homo.eps",transparent=True)
plt.savefig(basename + ".homo.svg",transparent=True)
plt.savefig(basename + ".homo.png",transparent=True)

fig = plt.figure(figsize=(10, 8))
ax = fig.add_subplot(111, projection='3d')

x = df['pm6_homo'].to_numpy().reshape(1,-1)[0]
y = df['b3lyp_pm6_homo'].to_numpy().reshape(1,-1)[0]
...
ax.set_title("PubChemQC B3LYP/6-31G*//PM6 HOMO",y=1.0, pad=-30)
ax.set_xlabel("HOMO by PM6//PM6 in eV", fontsize=18, labelpad=30)
ax.set_ylabel("HOMO by B3LYP/6-31G*//PM6 in eV", fontsize=18, labelpad=30)
...
for ii in range(45,46,10):
    ...
    plt.savefig(basename + ".homo.3d.%03d.eps" % ii ,transparent=True)
    plt.savefig(basename + ".homo.3d.%03d.svg" % ii ,transparent=True)
    plt.savefig(basename + ".homo.3d.%03d.png" % ii ,transparent=True)
